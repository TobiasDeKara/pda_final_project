...
#run models, all 50 states, integer restrictions
countydemanalysis <- df %>% filter(hasabortiondata == 1)

ext_model_dist <- lm(AbortionRate2010 ~ totalrestrict + distancetoprovider + totalrestrict*distancetoprovider, data = countydemanalysis, weights = ps)
summ_ext_dist <- summary(ext_model_dist)

ext_model_no_dist <- lm(AbortionRate2010 ~ totalrestrict, data = countydemanalysis, weights = ps)
summ_ext_no_dist <- summary(ext_model_no_dist)

# Model Summaries (Note coefficients are per 1000 women)
table_ext_no_dist <- tibble(Characteristic = "Number of Types of Restrictive Legislation",
                  Beta = ext_model_no_dist$coefficients[2]*1000,
                  "p-value" = summ_ext_no_dist$coefficients[2,4]) %>% 
  kbl(caption = "Table ****: Change in Abortions per 1000 Women in Model without Distances, all 50 States",
          digits = 3)
print(table_ext_no_dist)

table_ext_dist <- tibble(Characteristic = c("Number of Types of Restrictive Legislation", 
                                            "Distance to Provider", 
                                            "Distance/Restriction Interaction"),
                  Beta = ext_model_dist$coefficients[2:4]*1000,
                  "p-value" = summ_ext_dist$coefficients[2:4,4]) %>% 
  kbl(caption = "Table ****: Change in Abortions per 1000 Women in Model with Distances, all 50 States", digits = 3)
print(table_ext_dist)
